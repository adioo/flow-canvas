...
module.exports = function () {
    ...
    this.network.on('afterDrawing', (ctx) => {
        ...
        if (this.menu.type === 'node') {
            ...
            // }
            let pos = this.network.getPositions(this.menu.node);
            this.menu.center = pos[this.menu.node];
        } else if (this.menu.type === 'edge') {
            let edge = this.network.body.edges[this.menu.edge];
            this.menu.center = edge.edgeType.getPoint(0.5);
        }
...
function createButtons (menu, config) {
    ...
    switch (menu.type) {
        ...
        case 'node':
            ...
            break;
        case 'edge':
            let btn = menu.buttons[0];
            if (!btn.ref) {
                btn.ref = new Button(menu.center.x, menu.center.y, config.buttonRadius, {
                    color: (btn.color && config.colors[btn.color]) ? config.colors[btn.color] : {},
                    icon: btn.icon
                });
            } else {
                btn.ref.x = menu.center.x;
                btn.ref.y = menu.center.y;
            }
